...
def get_test_cases():
    """Collect test cases for problem-1 and problem-2 only"""
    test_data = []
    
    # Test problem-1 (src) -> problem-2 (tests)
    # Test problem-2 (src) -> problem-3 (tests)
    problem_indices = [1, 2]
    ...
    for i in problem_indices:
        ...
        spec = importlib.util.spec_from_file_location(f"zephyr_scot_{i}", problem_path)
...
def test_zephyr_solution(problem_name, solve, test_case, test_index):
    """Test Zephyr solutions against test cases"""
    try:
